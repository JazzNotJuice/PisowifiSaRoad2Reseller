...
<script>
    $(document).ready(function(){
		...
		const url = 'https://script.google.com/macros/s/AKfycbzjze5AB2y5EgDc7I8WJ8MwUmgUpPdiKLofYKR5ugma/exec?code=<? echo $uname; ?>&date=<?php echo date("Y-m-d"); ?>&time=<?php echo date("h:i a"); ?>&price=<?php echo $getprice; ?>&storename=Store1';
		// Create an invisible image to load the URL in the background
		const img = new Image();
		img.src = url;
			...
			//		sendToQuickPrinterChrome();
  			//} else if (w > 800) {
					...
					window.open('./voucher/print.php?user=<?= $usermode ?>-<?= $uname ?>&qr=<?= $qr ?>&session=<?= $session ?>','_blank','width=310,height=450');
